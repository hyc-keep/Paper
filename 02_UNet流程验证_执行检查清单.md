...
为了避免后面把 02 的工作全部堆到服务器上，这里单独把边界写清楚。

### 13.1 当前服务器只负责哪一段

在当前 `02_UNet流程验证` 收口里，服务器只负责 GPU 依赖部分：
...
**服务器只负责把严格规划要求的 GPU 主证据补齐。**

### 13.2 服务器做完后本地 CPU 负责哪一段

当正式 GPU A1 和关键结果证据已经补齐后，本地 CPU 可以继续完成：

1. 入口 README 改写
2. `implementation_status.md` 改写
3. “当前阶段为什么能pass以及下一步怎么看” 改写
4. `run_meta.yaml`、`run_summary.md` 与状态页一致性核对
5. learning docs 对照
6. 最终复审
7. 正式写出“允许进入 `03_UNet稳定性`”的结论

这些工作不再依赖 GPU 算力。

### 13.3 什么时候可以先离开服务器

只要下面这些条件已经成立，就可以先结束当前 02 阶段的服务器使用：

1. GPU smoke check 已通过
2. 正式单 GPU + AMP A1 已完成
3. `run_meta.yaml` 与 `run_summary.md` 已落盘
4. `TestA60`、`TestB20` 与 `metric_crosscheck` 已完成
5. 当前结论所需的关键结果已经能回填到 Git 文本基线

满足这些条件后，剩余的 02 阶段收口工作原则上可以回到本地 CPU 完成。

### 13.4 需要特别防止的误解

这里最容易出现的误解是：

- “本地 CPU 足够完成进入 03 的收尾工作”

被误听成：

- “03 阶段正式实验以后也都不需要 GPU”

这两句话不是一回事。

更准确的说法是：

**本地 CPU 足够完成 02 阶段剩余收口并正式进入 03；但 03 阶段真正开始做稳定性重复实验时，通常仍需要重新回到 GPU 环境。**
